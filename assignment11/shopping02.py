...
async def checkout_customer(queue: Queue, cashier_number: int):
    ...
    while not queue.empty():
        ...
        cashier_status["time"] += round(product.checkout_time, ndigits=2)
...
async def customer_generation(queue: Queue, customers: int):
    customer_count = 0
    while True:
        gen_customers = [generate_customer(the_id) for the_id in range(customer_count, customer_count+customers)]
        for customer in gen_customers:
            print("Waiting to put customer in line....")
            await queue.put(customer)
            print("Customer put in line....")
        customer_count = customer_count + len(gen_customers)
