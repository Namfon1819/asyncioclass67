...
async def ApplyButter():
  ...
  await asyncio.sleep(1)  # Simulate asynchronous wait for buttering
...
async def FryEggs(eggs):
  ...
  await asyncio.sleep(1)  # Simulate asynchronous wait for pan heating
  for egg in range(eggs):
    print(f"{time.ctime()} - Frying", egg+1,"eggs")
    await asyncio.sleep(1)  # Simulate asynchronous wait for each egg to fry
  print(f"{time.ctime()} - Finish fry eggs...")
  print(f"{time.ctime()} - >>>>>>>>>> Fry eggs are ready...")

async def FryBacon():
  print(f"{time.ctime()} - Begin fry bacon...")
  await asyncio.sleep(2)  # Simulate asynchronous wait for bacon to fry
...
async def ToastBread(slices):
  for slice in range(slices):
    ...
    await asyncio.sleep(1)  # Simulate asynchronous wait for each slice to toast
    print(f"{time.ctime()} - Bread", slice + 1, "toasted")
    await ApplyButter()  # Call ApplyButter asynchronously
...
async def main():
  ...
  tasks = [
      ...
      asyncio.create_task(ToastBread(2))  # Simulate making 2 slices of toast
  ]
